package com.redeyesncode.redbet.session

object Constant {
    val PREFERENCES_NAME: String = "PAY2KART_SESSION"
    val USER_LOGIN: String = "USER_LOGIN"
    val USER_LOGIN_CRM: String = "USER_LOGIN_CRM"
    val EMP_ID:String = "EMP_ID"
    val MPASS:String = "MPASS"
    val CUSTOM_PACKAGE_ID: String = "CUSTOM_PACKAGE_ID"
    val REGULAR_PACKAGE_ID: String = "CUSTOM_PACKAGE_ID"
    val USER_LOAN_ID: String = "USER_LOAN_ID"
    val BODY_CREATE_LEAD: String = "BODY_CREATE_LEAD"
    val RESPONSE_CREATE_LOAN_USER: String = "RESPONSE_CREATE_LOAN_USER"
    val BODY_APPLY_KYC: String = "BODY_APPLY_KYC"
    val BODY_UPDATE_KYC: String = "BODY_UPDATE_KYC"
    val USER_DASHBOARD: String = "USER_DASHBOARD"

    val IS_LOGGED_IN:String = "IS_LOGGED_IN"

}