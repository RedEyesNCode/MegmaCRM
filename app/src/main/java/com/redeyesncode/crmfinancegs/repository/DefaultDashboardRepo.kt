...
class DefaultDashboardRepo : DashboardRepo {
    ...
    override suspend fun getLeadEmi(map: HashMap<String, String>): Resource<ResponseLeadEMI> {
        return safeCall {
            safeCall {
                val response =
                    RetrofitInstance().provideApiService(RetrofitInstance().provideRetrofit()).getLeadEmi(map)
